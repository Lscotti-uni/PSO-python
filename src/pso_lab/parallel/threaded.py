"""Thread-pool evaluator for concurrency experiments under the GIL."""
...
class ThreadPoolEvaluator:
    ...
    def __init__(self, workers: int | None = None, batch_size: int = 1) -> None:
    ...
    def evaluate(self, positions: np.ndarray, objective: ObjectiveFn) -> np.ndarray:
        ...
        for chunk_id, (start, stop) in enumerate(chunks):
            futures.append((chunk_id, self._executor.submit(_evaluate_chunk, positions[start:stop], objective)))

        results: list[np.ndarray | None] = [None] * len(chunks)
        worker_times: list[float] = []
        for chunk_id, future in futures:
            # Results are written back by chunk id so the returned fitness array
            # keeps the same particle order as the input swarm.
